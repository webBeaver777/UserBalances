<template>
  <div class="operations-table">
    ...
    <div v-else-if="operationStore.error" class="alert alert-danger" role="alert">
    ...
    <div v-else>
        ...
        <table class="table table-hover">
          ...
          <tbody>
            <tr v-for="operation in operationStore.operations" :key="operation.id">
              <td>
                <span class="badge" :class="operation.type === 'deposit' ? 'bg-success' : 'bg-warning'">
                  {{ operation.type === 'deposit' ? 'Пополнение' : 'Списание' }}
                </span>
              </td>
              <td class="amount" :class="operation.type === 'deposit' ? 'text-success' : 'text-danger'">
      ...
      <!-- Пагинация -->
      <div v-if="showPagination && operationStore.totalPages > 1" class="d-flex justify-content-between align-items-center mt-3">
        ...
        <nav aria-label="Пагинация операций">
          <ul class="pagination mb-0">
            <li class="page-item" :class="{ disabled: operationStore.currentPage === 1 }">
              <button class="page-link" @click="handlePageChange(operationStore.currentPage - 1)" :disabled="operationStore.currentPage === 1">
                Предыдущая
              </button>
            </li>
            <li
            ...
            <li class="page-item" :class="{ disabled: operationStore.currentPage === operationStore.totalPages }">
              <button class="page-link" @click="handlePageChange(operationStore.currentPage + 1)" :disabled="operationStore.currentPage === operationStore.totalPages">
                Следующая
              </button>
            </li>
          </ul>
